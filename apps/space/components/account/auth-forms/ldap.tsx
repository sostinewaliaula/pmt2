...
type TLDAPAuthForm = {
  handleAuthStep: (step: string) => void;
...
export const AuthLDAPForm = ({ handleAuthStep, handleErrorInfo }: TLDAPAuthForm) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (nextPath) formData.append("next_path", nextPath);

      // Submit to LDAP endpoint
      const response = await fetch(`${API_BASE_URL}/auth/spaces/ldap/`, {
        method: "POST",
        body: formData,
        credentials: "include",
      });
